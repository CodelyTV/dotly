...
skills::dump() {
  ...
  # Use Python to parse the JSON and produce YAML output
  local python_cmd=""
  ...
  if [[ -z "$python_cmd" ]]; then
    _dump_log "error" "No python found — writing empty lockfile."
    {
      yaml::write_value "format" "skill-lock-v1"
      echo "providers: []"
    } | yaml::write_document "$dump_file" "# Agent skills lockfile — generated by dot package dump"
...
skill_map = {}
for s in skills:
    skill_map[s.get('name', '')] = s

for name in agents:
    yaml::write_array_item('name: %s' % name, 1)
    print('    skills:')
    sdata = skill_map.get(name, {})
    cmd = sdata.get('provider', '') or sdata.get('command', 'unknown')
    branch = sdata.get('branch', '')
    if branch:
        cmd = '%s#%s' % (cmd, branch)
    agents_list = sdata.get('agents', [])
    print('        command: %s' % (cmd or 'unknown'))
    print('        agents:')
    if agents_list:
