...
for skill_name in sorted(skills_data.keys()):
    sdata = skills_data[skill_name]
    if not isinstance(sdata, dict):
        sdata = {}
    src = sdata.get('source', '') or sdata.get('provider', '') or 'unknown'
    skill_path = sdata.get('skillPath', '')
    if skill_path:
        cmd = '%s#%s' % (src, skill_path)
    else:
        cmd = src
    full_cmd = 'npx -y %s' % cmd
    print('  - name: %s' % skill_name)
    print('    skills:')
    print('      - name: %s' % skill_name)
